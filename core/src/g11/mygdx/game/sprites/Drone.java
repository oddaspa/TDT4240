...
public class Drone {
    ...
    public Drone(float x){
        rand = new Random();
        System.out.println("Genereating random Y");
        ...
        drone = new Texture("droneCycle.png");
    ...
    public void update(float dt) {
        ...
        if (position.y < startY - (drone.getHeight()/2)) {
            move();
        } else {
        ...
        velocity.scl(1/dt);
        bound.setPosition(position.x + drone.getWidth()/2, position.y);
    }
    ...
    public TextureRegion getTexture() {
        return droneAnimation.getFrame();
    }

    public void move(){
        //System.out.println("Drone did move();");
        velocity.y = 100;
        velocity.x = speed;
    }
